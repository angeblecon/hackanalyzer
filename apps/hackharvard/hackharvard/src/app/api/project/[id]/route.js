	...
	async function* handler(id) {
		...
		if (projectInfo.githubLink) {
			try {
				...
				for await (const githubInsights of iteratorToStream(
					getGithubInsights(owner, repo.replace(".git", ""))
				)) {
		...
		const similarProjects = await searchProjects(keywords.join(" ")).then((r) =>
			r.filter((p) => p.id !== projectInfo.id).map((project) => ({ project }))
		);
