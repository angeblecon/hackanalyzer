	...
	return (
		<div id="header-container">
			...
			<div id="picture-with-members">
				...
				{project.images.length > 0 && <img src={project.images[0]} id="project-img" />}
			</div>
			{project.structuredDescription?.shortDescription && (
				<span id="short-description">{project.structuredDescription.shortDescription}</span>
			)}
			{project.structuredDescription && (
				<div id="description">
					{Object.entries({
						...
						<span key={id}>
							<b>{text}</b>
							<span className="criteria-description">{project.structuredDescription[id]}</span>
						</span>
					))}
				</div>
			)}
